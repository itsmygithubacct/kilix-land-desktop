...
# The same colours the previous editor used, so a room looks the way the
# people who painted these rooms are used to.
DOOR_RGB = "78AAFF"
OBJECT_RGB = "FFD250"
NPC_RGB = "F050DC"
SPAWN_RGB = "5AE678"
...
def incoming_spawns(world, room_id):
    """Where the player arrives in this room, which is a property of the
    doors in *other* rooms - and exactly the thing you cannot see while
    painting this one."""
    points = []
...
def marks_for_room(world, room):
    """The room's context as kilix-mask annotation lines, in plate pixels.

    Everything here is drawn *around*, never painted: doors, the objects
    the player must not walk through, where characters stand and where
    they and their items arrive.  Painting walkable space without them
    means painting blind and hearing about it from the validator.
    """
    lines = ["# generated by tools/region_editor.py; not part of the mask"]

    def px(value):
        return int(round(value * PLATE_W / LOGICAL_W))

    def py(value):
        return int(round(value * PLATE_H / LOGICAL_H))

    def rect(r, colour, label):
        lines.append(f"rect {px(r['x'])} {py(r['y'])} "
                     f"{px(r['w'])} {py(r['h'])} {colour} {label}".rstrip())

    def point(x, y, colour, label):
        lines.append(f"point {px(x)} {py(y)} {colour} {label}".rstrip())

    for door in room.get("doors", []):
        rect(door["rect"], DOOR_RGB, f"door {door.get('to', '')}".strip())
    for entry in room.get("objects", []):
        label = entry["id"]
        if "receiver" in entry:
            label += f" [R {entry['receiver']}]"
        rect(entry["rect"], OBJECT_RGB, label)
    for npc in room.get("npcs", []):
        point(npc["x"], npc["y"], NPC_RGB, f"npc {npc.get('actor', '')}")
    for x, y, label in incoming_spawns(world, room["id"]):
        point(x, y, SPAWN_RGB, label)
    for spawn in room.get("item_spawns", []):
        point(spawn["x"], spawn["y"], ITEM_RGB, item_spawn_label(spawn))
    return "\n".join(lines) + "\n"
